...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Remove { input, config, email, password } => {
            remove_command(&input, &config, &email, &password)?;
        }
        Commands::Export { input, output, config } => {
...
fn analyze_command(input: &str, config_path: &str) -> Result<()> {
    ...
    let results: Vec<_> = connections.iter().map(|c| filter_engine.filter_connection(c)).collect();
